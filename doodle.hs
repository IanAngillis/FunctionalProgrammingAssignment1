module Doodle (Doodle(initialize, add, remove, toggle), Pool(freshKey, get, set), run) where
...
participate n d = putStrLn (show d) >> prompt "Toggle a slot?" (participate n d) f
  where f Nothing  = putStrLn "Thanks for participating!" >> return d
        f (Just i) = participate n (toggle n i d)
